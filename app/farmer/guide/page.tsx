...
export default function FarmingGuide() {
  ...
  async function askQuestion(e: React.FormEvent) {
    ...
    if (data.error) {
      ...
      // log it for later review, matching FARMER_QUERIES table
      const { data: { user } } = await supabase.auth.getUser()
      if (user) {
  ...
  const grouped = stageOrder
    .map(stage => ({ stage, items: tips.filter(t => t.growth_stage === stage) }))
  ...
  return (
    <div className="max-w-xl mx-auto mt-12 p-6">
      <h1 className="text-2xl font-bold text-green-800 mb-1">Farming Guide — Wheat</h1>
      <p className="text-gray-500 text-sm mb-6">
        Practical tips by growth stage, adapted from ICAR and established IPM practice.
      </p>
            ...
            {g.items.map(tip => (
              <div key={tip.id} className="border rounded p-3">
                <span className={`text-xs font-semibold px-2 py-0.5 rounded ${
                  ...
                  {tip.practice_type === 'organic' ? '🌱 Organic-first' : '🧪 Conventional (ICAR)'}
                </span>
                <p className="text-sm mt-2">{tip.tip_content_en}</p>
                <p className="text-xs text-gray-400 mt-1">Source: {tip.source_basis}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="border-t pt-6 mt-6">
        <h2 className="text-lg font-bold text-green-800 mb-2">Ask a question</h2>
      ...
      <p className="text-xs text-gray-400 mt-8 border-t pt-4">
        These guides are general agricultural information adapted from ICAR and established IPM
        practice. They are not a substitute for your local Krishi Vigyan Kendra's advice, product
        labels, or a soil test. Always follow product label instructions for any chemical input.
      </p>
    </div>
  )
}