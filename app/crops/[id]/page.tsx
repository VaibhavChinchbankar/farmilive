...
export default function CropDetail() {
  ...
  async function handleReserve() {
    ...
    if (!profile || profile.role !== 'customer') {
      setMessage(
        `⚠️ You're logged in as a "${profile?.role || 'unknown'}" account. Please log out and log in with a customer account to reserve a crop.`
      )
      return
    }
  ...
  return (
    <div className="max-w-2xl mx-auto mt-12 p-6">
      ...
      <div className="border rounded p-4 mb-6 text-sm space-y-1">
        <p><b>Sowing date:</b> {crop.sowing_date}</p>
        <p><b>Expected harvest:</b> {crop.expected_harvest_date}</p>
        <p><b>Expected allocation for you:</b> ~{perSubAllocation} kg</p>
        <p><b>Slots:</b> {crop.slots_sold} / {crop.slots_total} reserved</p>
      </div>
      ...
      <div className="border rounded p-4 bg-amber-50 mb-4">
        <label className="flex gap-2 text-sm items-start">
          ...
          <span>
            I understand this is an <b>expected allocation</b>. Final quantity depends on the
            actual verified harvest, per the published formula:
            <br /><i>Payout = min(Actual, Expected) × Price + Shortfall × Refund%</i>.
            I pay only after harvest, plus a refundable ₹99 booking token now.
          </span>
        </label>
      </div>
      ...
      <button
        ...
        {reserving ? 'Reserving...' : 'Reserve with ₹99 (refundable)'}
      </button>

      {message && <p className="mt-4 text-sm">{message}</p>}

      <h2 className="text-lg font-bold text-green-800 mt-10 mb-3">Verified Crop Updates</h2>
      {updates.length === 0 && (
        <p className="text-gray-500 text-sm">No verified updates yet — check back soon.</p>
      )}
            ...
            <div className="p-3">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-xs text-gray-400">
                  {new Date(u.server_time).toLocaleDateString()}
                </span>
              </div>
              <p className="text-sm text-gray-700">{u.note}</p>
              <p className="text-xs text-gray-400 mt-1">📍 Farm location verified</p>
            </div>
          </div>
        ))}
      </div>
