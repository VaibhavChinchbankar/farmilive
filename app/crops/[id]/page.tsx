...
export default function CropsPage() {
  ...
  if (loading) return <p className="text-center mt-16">Loading crops...</p>

  return (
    <div className="max-w-4xl mx-auto mt-12 p-6">
      <h1 className="text-2xl font-bold text-green-800 mb-2">Explore Crops</h1>
      <p className="text-gray-600 mb-6">Choose a crop, follow its journey, own the harvest.</p>

      {crops.length === 0 && <p className="text-gray-500">No approved crops yet — check back soon.</p>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {crops.map(crop => (
          <Link key={crop.id} href={`/crops/${crop.id}`}
            ...
            <p className="text-sm text-gray-500 capitalize">🌱 Current stage: {crop.current_stage}</p>
            <p className="text-sm text-gray-500">📅 Expected harvest: {crop.expected_harvest_date}</p>
            <p className="text-sm text-green-700 font-semibold mt-2">
              {crop.slots_sold} / {crop.slots_total} slots reserved
            </p>
          </Link>
        ))}
      </div>
